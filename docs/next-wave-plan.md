...
| 11 | Performance | Done | Light problem index, per-route picks (daily problem, leaderboard scoring), a lazy Zero mount, and an on-demand problem bank (24 h ISR; build output ~136 MB, was ~3.5 GB); home 276.6 KB gzip (from 1,553 KB); gzip budgets enforced by `scripts/measure-bundle.ts --check` (4/4 PASS) |
