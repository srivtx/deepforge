...
| 2 | Quality gates | Done, in CI | `verify-problems.ts` (real Python) + `verify-paths.ts` (capstones 33/33) + `verify-paths-content.ts`; 1,582 `bun test` greens (78 files); GitHub Actions (`.github/workflows/ci.yml`) runs every gate, the build, and the 187-check e2e smoke |
| 3 | Routes | Done | 33 user-facing destinations incl. `/today`, `/review`, `/alibi`, `/ledger`, `/inventions`, `/verify`, `/concepts`, and `/papers`; lab/research detail pages + `/labs/trails`; projects have detail pages at `/projects/[id]`; section modals retired; home is a short landing |
...
| 14 | PWA / offline | Done | SW v9: per-route offline fallback verified by a route-inventory test (`tests/offline.test.ts`, which now models nested static routes) + “update available” prompt (`PwaManager.tsx`); `/labs/trails`, `/concepts`, `/papers`, `/review`, `/inventions`, `/alibi`, and `/ledger` are precached and `/labs/`, `/research/`, `/papers/`, `/inventions/`, and `/ledger/` have navigation fallbacks |
