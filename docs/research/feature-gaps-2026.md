...
4. **`src/app/sitemap.ts:15` hardcodes `LAST_MODIFIED = new Date("2026-09-01…")` for ~5,600 problem URLs plus all routes.** Every page claims the same last-modified date; `lastmod` is therefore meaningless to crawlers. Its own plan flagged this (NW-06) and it is still open.
