...
- **REPROGPU** — `/reprogpu` is a read-only WebGPU conformance lab: it runs a declared integer kernel subset (a 320-bit exact f32 sum, Philox4x32-10, Q16.16 GEMM, integer SHA-256) in your browser and compares each output's SHA-256 against the pinned CPU reference, publishing an adapter manifest so third parties can compare across adapters; an f32 matmul is the explicit negative control. The sixth house paper documents the WGSL exactness boundary (float is outside the guarantee), the pinned reference hashes, and the honesty rule that CI checks the references and vectors while the browser run is the cross-adapter demonstration. Scope: this is a cross-adapter *artifact/conformance* contribution, not a novel method — the exact-integer-hashed-and-compared idea already exists live (Coherence `gpu-exact`), and the reproducible-accumulation science is mature
