...
- ✅ Wave 48: REPROGPU — cross-adapter bit-reproducible WebGPU conformance lab for a declared integer kernel subset (320-bit exact f32 sum, Philox4x32-10 against Random123 KATs, Q16.16 GEMM with a single floor shift, integer SHA-256 against FIPS/Node oracle), with an f32 matmul negative control and the WGSL exactness boundary documented (float is outside the guarantee); pure-TS reference gate (5 criteria, ~0.8 s) in CI, browser harness manifests adapter info + per-kernel SHA-256; `/reprogpu` lab + paper #6. Scope: **artifact/conformance contribution, GO-WEAK — not a novel method** (exact-integer-hashed-and-compared already exists live in the Coherence `gpu-exact` demo; bitgpu ships bit-exact browser WebGPU integer execution); the prior-art miss and the retired "unmatched" wording are recorded in `docs/research/wave-48-priorart-reprogpu.md` §10 and in the paper's §2
