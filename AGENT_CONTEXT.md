...
- ✅ PWA offline: route fallback chain + “update available” prompt (SW v8) with per-route fallback verified by a route-inventory test
...
- ✅ CI: GitHub Actions runs typecheck, lint, unit tests, all verifiers, the build, and the 179-check smoke
...
5. **E2E in CI** — shipped: the 179-check smoke runs in GitHub Actions; remaining: live-preview checks after deploy.
...
- **Tests:** ✅ 1,417 unit tests (74 files) + 179 e2e smoke checks (CI)
