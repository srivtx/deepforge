...
- ✅ PWA offline: route fallback chain + “update available” prompt (SW v9) with per-route fallback verified by a route-inventory test
...
- ✅ CI: GitHub Actions runs typecheck, lint, unit tests, all verifiers, the build, and the 187-check smoke
...
5. **E2E in CI** — shipped: the 187-check smoke runs in GitHub Actions; remaining: live-preview checks after deploy.
...
- **Tests:** ✅ 1,582 unit tests (78 files) + 187 e2e smoke checks (CI)
