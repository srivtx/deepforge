  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          kind: "paragraph",
          text: "The curated set is the census turned into content: 32 crash-divergence witnesses and 64 value-divergence witnesses, no timeouts; difficulty Easy 24, Medium 49, Hard 23; all 15 categories represented, 3-10 puzzles each; 412 shipped tests, every one passed by both the reference and the ghost; exactly one changed line in all 96, because the stored references are ast.unparsed so formatting is identical; and 96/96 re-verified for the five load-bearing claims (reference passes the shipped tests, ghost passes the shipped tests, ghost diverges from reference at the stored witness under a 1e-6 comparison and a 0.25 s alarm, and ghost diverges on zero of its union probes and zero of its held-out probes). The whole set serializes to 152,696 bytes raw and 27,510 bytes gzip -9. The permanent gate scripts/verify-alibis.ts plus scripts/py_alibi_verify.py re-checks every claim, including both resistance suites, outside the browser and exits non-zero on any failure; the last gate run passed 96/96 with zero union, held-out, metadata, or witness failures.",
        },
        {
          kind: "paragraph",
